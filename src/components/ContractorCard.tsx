...
function ContractorForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <div>
        <label className="block text-sm text-slate-300 mb-1">Navn *</label>
        <input
          type="text"
          value={formData.name}
      ...
      <div>
        ...
        <div className="flex gap-2 w-full">
          <select
            ...
            className="px-2 py-2 bg-slate-800 border border-slate-600 rounded-lg text-white focus:ring-2 focus:ring-sky-500 focus:border-sky-500 shrink-0"
          >
            {countryCodes.map((cc) => (
          ...
          <input
            ...
            className="flex-1 min-w-0 px-3 py-2 bg-slate-800 border border-slate-600 rounded-lg text-white focus:ring-2 focus:ring-sky-500 focus:border-sky-500"
      ...
      <div>
        <label className="block text-sm text-slate-300 mb-1">E-post (valgfritt)</label>
        <input
          type="email"
      ...
      <div className="flex gap-3 pt-2">
        <button
          ...
          className="flex-1 px-4 py-2 bg-sky-600 text-white rounded-lg hover:bg-sky-500 disabled:opacity-50 disabled:cursor-not-allowed"
        >
...
function ContractorItem({
  ...
  return (
    <div className={`bg-slate-800 rounded-lg p-4 ${contractor.isPrimary ? 'ring-2 ring-sky-500' : ''}`}>
      <div className="flex justify-between items-start mb-2">
        <div>
          <div className="flex items-center gap-2">
            <p className="font-medium text-white">{contractor.name}</p>
            {contractor.isPrimary && (
              <span className="text-xs bg-sky-600 text-white px-2 py-0.5 rounded-full">Favoritt</span>
            )}
          </div>
        ...
        <div className="flex gap-1">
          {!contractor.isPrimary && (
            <button
              ...
              className="p-1.5 text-slate-400 hover:text-yellow-400 hover:bg-slate-700 rounded"
      ...
      <div className="flex gap-2 pt-2">
        <button
          onClick={handleCall}
          className="flex-1 px-3 py-1.5 bg-green-600 text-white text-sm rounded-lg hover:bg-green-500"
        >
          📞 Ring
        </button>
        <button
          onClick={handleSms}
          className="flex-1 px-3 py-1.5 bg-sky-600 text-white text-sm rounded-lg hover:bg-sky-500"
        >
          💬 SMS
        </button>
        {contractor.email && (
